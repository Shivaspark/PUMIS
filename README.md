...
`dpumis.py` - Main Streamlit web application and ContextGAN architecture

`tstr_evaluator.py` - Standalone Machine Learning Efficacy (TSTR) testing script

`pumis_generator.pth` - Pre-trained Generator weights (NYC TLC Model)

`scaler_params.pth` - Saved state dictionary for data normalization/scaling

`original_nyc_data.csv` - Ground truth raw dataset (downloaded via script)

`pumis_synthetic_clone.csv` - Generated private synthetic data output

`README.md` - Main GitHub repository documentation

`requirements.txt` - Project dependencies (PyTorch, Streamlit, pandas, etc.)
## 👥 Authors
